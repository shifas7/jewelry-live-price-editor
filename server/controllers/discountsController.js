...
export async function createDiscount(req, res) {
  try {
    ...
    if (
      !application_type ||
      !["collection", "products"].includes(application_type)
    ) {
      return res.status(400).json({
        success: false,
    ...
    const discountData = {
      ...
      application_type,
      target_collection_id:
        application_type === "collection" ? target_collection_id : null,
      target_product_ids:
        application_type === "products" ? target_product_ids : [],
      gold_rules,
      diamond_rules,
      silver_rules,
      is_active,
    ...
    if (
      ...
      result.metaobjectCreate.userErrors.length > 0
    ) {
      return res.status(400).json({
        success: false,
        error: result.metaobjectCreate.userErrors
          .map((e) => e.message)
          .join(", "),
      });
    }
    ...
    if (!result.metaobjectCreate?.metaobject) {
      return res.status(500).json({
        success: false,
        error:
          "Failed to create discount. Metaobject definition might not exist in Shopify.",
      });
    }
    ...
    const applyResult =
      await applicationEngine.applyDiscountToTargets(createdDiscount);
...
export async function updateDiscount(req, res) {
  try {
    ...
    const discountData = {
      ...
      is_active:
        is_active !== undefined ? is_active : existingDiscount.is_active,
    };
    ...
    if (
      ...
      result.metaobjectUpdate.userErrors.length > 0
    ) {
      return res.status(400).json({
        success: false,
        error: result.metaobjectUpdate.userErrors
          .map((e) => e.message)
          .join(", "),
      });
    }
    ...
    const targetsChanged =
      ...
      (discountData.application_type === "collection" &&
        existingDiscount.target_collection_id !==
          discountData.target_collection_id) ||
      (discountData.application_type === "products" &&
        JSON.stringify(existingDiscount.target_product_ids) !==
          JSON.stringify(discountData.target_product_ids));

    if (targetsChanged) {
      applicationEngine.init();
      const oldProductIds =
        await applicationEngine.getTargetProductIds(existingDiscount);
    ...
    const applyResult =
      await applicationEngine.applyDiscountToTargets(updatedDiscount);
  ...
  try {
    ...
    try {
      ...
      const targetProductIds =
        await applicationEngine.getTargetProductIds(discountToDelete);
    ...
    if (
      ...
      result.metaobjectDelete.userErrors.length > 0
    ) {
      return res.status(400).json({
        success: false,
        error: result.metaobjectDelete.userErrors
          .map((e) => e.message)
          .join(", "),
      });
    }
...
export async function applyBulkDiscount(req, res) {
  try {
    ...
    const results = await shopifyAPI.bulkApplyDiscount(
      productIds,
      discountConfig,
      priceCalculator,
    );
...
export async function applyCollectionDiscount(req, res) {
  try {
    ...
    // Get all products in collection (handles pagination)
    const products = await shopifyAPI.getAllCollectionProducts(collectionId);
...
export async function resolveConflict(req, res) {
  try {
    ...
    const result = await applicationEngine.resolveConflict(
      productId,
      discount,
      action,
    );
